...
- **Chart uses hardcoded dummy data** — `src/components/Swap/Chart.jsx` renders a DevExpress area chart with 12 months of fake static data. Pre-existing from the Splassive fork, not a modernization regression. Needs real price feed data.
...
- **CSS bundle size (504 KB)** — Bootstrap (full CSS imported) and DevExpress chart styles contribute significant weight. Consider PurgeCSS, importing only used Bootstrap component CSS, or replacing DevExpress with a lighter chart library.
